...
(2) RAGAS 채점 (LLM-as-judge: faithfulness, 선택적으로 answer_relevancy·context_precision)
...
context_recall은 정답(reference)이 필요해 golden set에 reference가 채워진 뒤 추가한다
...
def run_ragas(
    ...
    (질문·검색 컨텍스트·응답만으로 LLM judge가 채점). context_recall은 정답(reference)이
    필요해 golden set에 reference가 채워질 때까지 미포함한다 — eval/golden_set/README 참고.
    """
    ...
    if with_contexts:
        ...
        # (context_recall은 reference 필수 → golden set에 정답이 채워지면 동일 패턴으로 추가)
