...
(2) RAGAS 채점 (LLM-as-judge: faithfulness, 선택적으로 answer_relevancy·context_precision·context_recall)
...
실행 예시:
    ...
    python eval/run_benchmark.py --with-contexts  # context_precision 추가 (reference 불필요)
    python eval/run_benchmark.py --with-recall    # reference가 있는 케이스만 context_recall 추가

지표 확장 메모(루브릭 #8): context_precision은 reference 없이 동작해 바로 추가했다.
context_recall은 정답(reference)이 필요하므로 reference가 채워진 케이스에 한해 실행한다
(eval/golden_set/README.md 참고). 모든 RAGAS 지표는 OPENROUTER_API_KEY 필요.
...
def run_ragas(
    ...
    """RAGAS faithfulness(기본) + answer_relevancy·context_precision(선택)을 채점한다.

    judge 없으면 None. `with_contexts`는 reference 없이 동작하는 context_precision을 추가한다
    (질문·검색 컨텍스트·응답만으로 LLM judge가 채점). `with_recall`은 정답(reference)이
    채워진 케이스만 대상으로 context_recall을 추가한다 — eval/golden_set/README 참고.
    """
    valid = [r for r in records if r.error is None and r.contexts]
    if with_recall:
        valid = [r for r in valid if r.reference]
    if not valid:
        return None
    ...
    if with_contexts:
        # context_precision: 검색된 컨텍스트가 응답에 실제로 기여했는지(상위 정렬 품질)를
        # reference 없이 judge가 평가한다. retrieved_contexts만 있으면 되므로 현 데이터셋으로 동작.
        ...
        [
            {
                ...
                **({"reference": r.reference} if with_recall and r.reference else {}),
            }
            for r in valid
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--with-contexts",
        action="store_true",
        help="context_precision(reference 불필요) 추가 채점",
    )
    parser.add_argument(
        "--with-recall",
        action="store_true",
        help="reference가 있는 케이스만 context_recall 추가 채점",
    )
    ...
    if not args.skip_ragas:
        ...
        try:
            ragas_scores = run_ragas(
                records,
                with_relevancy=args.with_relevancy,
                with_contexts=args.with_contexts,
